...
cst816t::cst816t(int sda, int scl, int8_t _rst, int8_t _irq)
{
    rst = _rst;
    _sda = sda;
    _scl = scl;
...
void cst816t::begin(touchpad_mode tp_mode)
{
    ...
    if (i2c_read(CST816T_ADDRESS, REG_CHIP_ID, dta, sizeof(dta)))
        Serial.println("CST816T i2c error");
    chip_id = dta[0];
...
void cst816t::begin(TwoWire *_wire, touchpad_mode tp_mode)
{
    uint8_t dta[4];
    if (rst >= 0)
    {
...
{
    uint8_t dta[6];
    if (!i2c_read(CST816T_ADDRESS, REG_GESTURE_ID, dta, sizeof(dta)))
    {
        gesture_id = dta[0];
        finger_num = dta[1];
