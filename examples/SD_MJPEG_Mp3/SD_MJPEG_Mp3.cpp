/*
 * @Description(CN):
 *      使用SD卡播放MJPEG格式文件 同时播放MP3文件
 *
 * @Description(EN):
 *       Translate to English: Play MJPEG format file using SD card while playing MP3 file
 *  simultaneously.
 *
 * @version: V1.0.0
 * @Author: LILYGO_L
 * @Date: 2023-09-22 17:37:56
 * @LastEditors: LILYGO_L
 * @LastEditTime: 2023-11-22 15:32:25
 * @License: GPL 3.0
 */
...
#define AUDIO_FILENAME "/44100Hz_32k.mp3"

// #define MJPEG_FILENAME "/240x240px_60fps.mjpeg"
#define MJPEG_FILENAME "/240x240px_30fps.mjpeg"
#define FPS 30
...
void setup()
{
    // Disable low voltage detection to avoid infinite reboot.
    ...
    else
    {
        ...
        gfx->println("Start play video:" MJPEG_FILENAME);
        start_ms = millis();
        curr_ms = millis();
        next_frame_ms = start_ms + (++next_frame * 1000 / FPS / 2);
        gfx->println("Start play audio:" AUDIO_FILENAME);
        audio.connecttoFS(SD, AUDIO_FILENAME);
        xTaskCreatePinnedToCore(Task1, "Task1", 2000, NULL, configMAX_PRIORITIES - 1, NULL, 1);
